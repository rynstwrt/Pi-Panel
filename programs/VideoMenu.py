...
VIDEOS = {
    "galaxies": "assets/galaxies.mp4",
    "among us": "assets/amogus.gif",
    "UwU": "assets/uwu.gif",
    "cat": "assets/cat.gif",
    "trippy": "assets/trippy.mp4"
}
NUM_ROWS = 4
NUM_COLS = 3
    ...
    def create_buttons(self):
        ...
        for y in range(NUM_ROWS):
            for x in range(NUM_COLS):
                if button_index >= len(VIDEOS):
                    return

                button_name = list(VIDEOS.keys())[button_index]
                video_location = VIDEOS[button_name]

                button = QPushButton(button_name)
                button.setProperty("isContentButton", ["true"])
                button.clicked.connect(lambda e, video_location=video_location: FullscreenVideoWindow(video_location))
